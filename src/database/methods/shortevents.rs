...
pub fn insert(connection: &PgConnection, shortevent: &ShortEvent) -> Result<usize, diesel::result::Error> {
...
pub fn register_vote(connection: &PgConnection, shortEventId: u8, userId: u8) -> Result<usize, diesel::result::Error> {
    diesel::insert(short_event_votes)
        ...
        .into_columns((
                short_event_votes::user_id,
                short_event_votes::event_id
        ))
        .execute(connection)
}
...
pub fn check_votes(connection: &PgConnection) -> Result<usize, diesel::result::Error> {
     ...
     */
    match short_event_votes.select(event_id)
        .group_by(event_id)
        .filter(sql("COUNT(*) >= 3"))
